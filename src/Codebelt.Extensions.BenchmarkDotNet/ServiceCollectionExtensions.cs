...
namespace Codebelt.Extensions.BenchmarkDotNet
{
    ...
    public static class ServiceCollectionExtensions
    {
        ...
        /// <summary>
        /// Adds a benchmark workspace implementation of type <typeparamref name="TImplementation"/> to the specified <see cref="IServiceCollection"/>
        /// </summary>
        /// <typeparam name="TImplementation">The type that implements the <see cref="IBenchmarkWorkspace"/> interface.</typeparam>
        /// <param name="services">The <see cref="IServiceCollection"/> to add the services to.</param>
        /// <param name="setup">The <see cref="BenchmarkWorkspaceOptions" /> which may be configured.</param>
        /// <returns>The original <see cref="IServiceCollection"/> instance for chaining.</returns>
        /// <remarks>
        /// Validates the <paramref name="services"/> parameter and the provided <paramref name="setup"/> configurator.
        /// Registers <see cref="IBenchmarkWorkspace"/> as a singleton using <typeparamref name="TImplementation"/>,
        /// applies the provided configuration, and registers the resolved <see cref="BenchmarkWorkspaceOptions"/> as a singleton.
        /// </remarks>
        public static IServiceCollection AddBenchmarkWorkspace<TImplementation>(this IServiceCollection services, Action<BenchmarkWorkspaceOptions> setup = null) where TImplementation : class, IBenchmarkWorkspace
        {
            Validator.ThrowIfNull(services);
            Validator.ThrowIfInvalidConfigurator(setup, out var options);
            return services
                .AddSingleton<IBenchmarkWorkspace, TImplementation>()
                .Configure(setup ?? (_ => {}))
