...
- The solution is `Codebelt.Extensions.BenchmarkDotNet.slnx`. Central package versions are maintained in `Directory.Packages.props`.
...
- `Codebelt.Extensions.BenchmarkDotNet.slnx` is the solution used for local development.
...
```powershell
dotnet restore "Codebelt.Extensions.BenchmarkDotNet.slnx"
dotnet build "Codebelt.Extensions.BenchmarkDotNet.slnx" --configuration Release --no-restore
```
...
The CI test plan currently runs **4** project(s) and excludes **0** project(s). The workflow also has an optional macOS test job.
...
```powershell
dotnet pack "Codebelt.Extensions.BenchmarkDotNet.slnx" --configuration Release --no-restore
```
...
By contributing to Extensions for BenchmarkDotNet API by Codebelt, you agree that your contributions will be licensed under the MIT license.
